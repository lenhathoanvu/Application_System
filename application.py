...
load_dotenv()
genai.configure(api_key=os.get("API_KEY"))
...
e5 = pickle.load(open('models/e5_small.pkl', 'rb'))
...
def resume():
    if request.method == 'POST':
        ...
        job_title, company_name, companies = solve_job(description_text)
        ...
        if name in df_book['fb_username'].to_list():
            ...
            return render_resume_template(name, current_job, years_of_experience, education, skills, certification, desired_job_title, location, job_title, company_name, companies, book_name=None, book_author=None, book_description=None, book_id=None)

        # Render results to template
        return render_resume_template(name, current_job, years_of_experience, education, skills, certification, desired_job_title, location, job_title, company_name, companies, book_name, book_author, book_description, book_id)
...
def extract_resume_details(file):
    ...
    info = model.generate_content(f"""Screen this resume: {pdf_content}. List: name (just write fullname,
                                        just capitalize first letter), current job (under 3 words), years of experience (just 1 number), 
...
def solve_job(cv):
    ...
    cv_embedded = np.array(e5.encode(cv))

    for i in range(len(df)):
        vector_str = df['all_e5_small_embedded'][i]
        vector_list = ast.literal_eval(vector_str)
        tensor = np.array(vector_list)
        dot_product = np.dot(cv_embedded, tensor)
        norm_a = np.linalg.norm(cv_embedded)
        norm_b = np.linalg.norm(tensor)
    ...
    for company in company_name:
        company_no_space = ''.join(company.split())
        clean_company = company_no_space.lower()
        companies.append(clean_company)

    return job_title, company_name, companies
...
def render_resume_template(name, current_job, years_of_experience, education, skills, certification, desired_job_title, location, job_title, company_name, comapnies, book_name, book_author, book_description, book_id):
    """
    ...
    return render_template(
        ...
        job_title=job_title, company_name=company_name, companies=comapnies,
